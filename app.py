...
_CATEGORY_REQUIRED_KEYWORDS = {
    ...
    # score_product()のPEELING_NAME_KEYWORDS（app.py内「ピーリング強制判定」）と
    # 一致させておくこと。ずれると酵素ピーリング等が楽天厳格検索側でだけ
    # 誤って除外される（過去に実害があった不整合）。
    "ピーリング":    ["ピーリング", "ピール", "aha", "bha", "pha", "角質", "スクラブ", "酵素", "exfoliant", "peel"],
}
...
_CANDIDATE_CATEGORY_FORBIDDEN: dict[str, list[str]] = {
    ...
    "ピーリング": [
        ...
        # 「ウォッシュ」「フォーム」は毎日使う洗顔料を指す語のため除外する。
        # ただし「酵素洗顔」等は正当な週1ピーリング商品として扱われてきた実績があり
        # （過去に誤除外の実害あり）、「洗顔」単体はここに含めない。
        "ウォッシュ", "フォーム", "face wash", "cleanser", "cleansing",
        # 毛穴洗浄・角栓除去・保湿・鎮静が主目的の商品はピーリングではない
        # （多少の角質除去作用があっても分類しない）
        "毛穴洗浄", "角栓除去", "保湿", "鎮静",
    ],
    "パック": [
...
def infer_virtual_product_fields(name, category="", ingredient_focus="", purpose=""):
    ...
    rules = [
        ...
        {
            # score_product()のPEELING_INGREDIENTSに合わせ、酵素系ピーリングは
            # aha_bhaとは別の"enzyme"タグとして扱う
            "keywords": ["酵素", "パパイン", "papain", "ブロメライン", "bromelain", "プロテアーゼ", "protease"],
            "active": ["enzyme"],
            "functions": ["角質ケア", "毛穴ケア"],
            "focuses": ["enzyme"],
            "strength": {"enzyme": "medium"},
        },
        {
...
_WHY_BEST_PROMO_PATTERNS = [
    # 日付・時刻レンジ型のセール告知(例:「【9/19 00:00~9/24 23:59】」)。
    # clean_display_product_name()側で":"が空白へ置換された後の形
    # (例:「【9/19 00~9/24 59】」)にも対応できるよう、区切り文字を
    # "/"・":"・空白のいずれでも許容する。両側とも「数字+区切り+数字」
    # (日付や時刻の形)を要求することで、"150~200mL"のような容量レンジや
    # 型番中の数字は誤って削除しない。
    r'[\(（【\[][^\)）】\]]{0,6}\d{1,2}[／/:\s]\d{1,2}[^\)）】\]]{0,10}[~〜～][^\)）】\]]{0,6}\d{1,2}[／/:\s]\d{1,2}[^\)）】\]]{0,10}[\)）】\]]',
...
def normalize_result(raw_data, image_path=""):
    return {
        ...
        "ai_improvement_strategy": raw_data.get("ai_improvement_strategy", []),
        "improvement_priority": raw_data.get("improvement_priority", []),
        "premium_improvement_priority": raw_data.get("premium_improvement_priority", []),
...
def group_ranking_by_category(ranking, per_category_limit=10):
    ...
    result = []
    for cat in sorted(groups.keys(), key=lambda c: _PRODUCT_RANKING_CATEGORY_ORDER.get(c, 99)):
...
# 「よく提案される商品」ランキングページ専用の並び順。
# ルーティン内の時系列順（CATEGORY_ORDER, step_sort_key専用）とは別物のため
# 共用しない（例: パック・ピーリングはルーティンでは洗顔直後〜化粧水前後に
# 位置するが、ランキング一覧ではクリーム・日焼け止めの後に表示したい）。
_PRODUCT_RANKING_CATEGORY_ORDER = {
    "クレンジング": 1,
    # build_product_ranking() は「洗顔」を「洗顔料」に正規化するため両表記を登録する。
    # ずれると洗顔料が末尾(99)に飛ばされ指定順にならない。
    "洗顔": 2,
    "洗顔料": 2,
    "ブースター": 3,
    "化粧水": 4,
...
def normalize_serum_roles(data):
    ...
    ブースター選定はプロンプト側の【ブースター選定ルール】で詳細な判断基準
    （乾燥・角層の乱れ・刺激緩和・改善速度・総合判断等）をAIに与えており、
    購入判断はAIの文脈理解に委ねるべきもの。以前はここでpurposeの語句一致
    だけで強制的にroleを上書きしていたが、それではAIがルールに基づき
    booster/mainを判断しても、単純なキーワードマッチで覆されてしまう
    （プロンプトの判断基準が実質無効化される）。
    そのため、AIが既に有効な値（"main"/"booster"）を出力していればそれを
    尊重し、欠落・不正な値の場合のみキーワードからの推定でフォールバックする。
    """
    booster_keywords = [
        "浸透", "導入", "土台", "なじみ", "ブースト"
    ]
    ...
    for section in ["morning", "night"]:
        for step in data.get(section, {}).get("steps", []):
            ...
            role = step.get("role", "")
            if role in ("main", "booster"):
                continue

            purpose = str(step.get("purpose", ""))
            # roleが欠落・不正な場合のみキーワードからフォールバック推定
...
def step_sort_key(step):
    ...
    if use_timing in _TIMING_ORDER_OVERRIDE:
        ...
        # ただしrole=boosterは「洗顔→ブースター→化粧水→美容液」の順が仕様のため対象外
        # （ブースター選定ルールにより before_toner が正しい指定）。
        pos = (_TIMING_ORDER_OVERRIDE["after_toner"]
               if (category == "美容液" and use_timing == "before_toner" and role != "booster")
    ...
    # 後方互換: use_timing未指定のroute=boosterは before_toner 相当
    # （洗顔→ブースター→化粧水→美容液の順が仕様のため）
    if role == "booster":
        return (_TIMING_ORDER_OVERRIDE["before_toner"], 0, 0, 0, priority)
...
【ブースター選定ルール（role=boosterを使う場合の判断基準）】
以下のいずれかに該当する場合のみ追加を検討する。該当しなければ追加しない。
①乾燥・水分不足が強い（肌の水分量不足・インナードライ・バリア低下等）→ 水分保持・浸透感向上・後続保湿のサポートが目的。最優先。
②角層が乱れている（ごわつき・キメ乱れ・化粧水が入りにくい・肌が硬い等）→ 角層を整え後続ケアの効果を高める。
③レチノール/ビタミンC/アゼライン酸等の刺激成分を使う場合、鎮静・保湿系ブースターを提案してもよい。
④保湿美容液だけでは改善速度が遅いとAIが判断し、ブースター追加で改善効果が高まると判断できる場合。
⑤乾燥・毛穴・ハリ等、複数の改善軸に総合的に寄与すると判断できる場合。
出力しない条件（原則追加しない）: 肌状態が良好／現在のルーティンで十分／改善効果が小さい／コストに対する改善幅が少ない／同等機能を他アイテムが既に担っている。
使用タイミング: 洗顔→ブースター→化粧水→美容液→乳液・クリームの順（use_timing="before_toner"）。メーカー指定の使用順がある製品はそちらを優先する。
化粧水・美容液との関係（重要）: ブースターを追加しても化粧水・美容液は原則省略しない（ブースター=導入・水分通路を整える・柔軟化が役割、化粧水=水分補給・保湿、美容液=有効成分による治療的ケアで役割が異なる補助カテゴリのため）。省略できるのは①メーカーがその製品を1本で代替可能と位置付けている場合（例: 導入+化粧水/導入+美容液の一体型製品）②AIが高保湿導入美容液と高保湿化粧水のように機能重複と判断した場合③重ねすぎで刺激過多になると予想される場合、のみ。
選定時の評価項目: 肌改善効果・後続スキンケアとの相性・成分重複・刺激リスク・コストパフォーマンス・週間ルーティンとの整合性・他カテゴリで代替可能か、を総合評価する。
選定対象: 導入美容液/導入化粧水/導入エッセンス/導入オイル(乾燥肌向け)/発酵系導入アイテム/保湿導入アイテムのみ。「メーカーが導入用途として位置付けている製品」に限り、通常の美容液・化粧水を転用してブースター扱いにしない。

【ブースターの朝使用ルール】
ブースターは朝・夜ともに使用可能（夜専用ではない）。ただし朝はメイクとの相性・UV対策・時短・ベタつきも考慮して提案すること。
朝に積極的に提案する条件:
①朝から乾燥する・日中につっぱる・インナードライ等、乾燥しやすい → 保湿ブースターを提案。
②メイク崩れの原因が水分不足の場合 → 朝のブースター追加で改善が期待できる。
③角層が乱れている・化粧水が入りにくい等、肌がごわつく場合 → 朝も有効。
④赤み・軽い刺激・敏感肌等、鎮静したい場合 → 朝でも鎮静系ブースターは有効。
朝は基本提案しない条件:
①脂性肌・高温多湿・皮脂過多等、ベタつきやすい場合。
②モロモロ・ヨレ・ファンデ密着低下等、メイクとの相性が悪いと予想される製品。
③夜使用だけで改善効果がほとんど変わらない場合。
朝の使用順: 洗顔→ブースター→化粧水→美容液→乳液・クリーム→日焼け止め→メイク。
判断項目: 肌質・肌状態・朝夜それぞれの目的・季節・使用感・メイクとの相性・他アイテムとの重複・コストパフォーマンスを総合評価する。
...
- ピーリングスロット→ 古い角質を除去しターンオーバー促進を主目的とする製品のみ。判断は①メーカーがピーリング/角質ケア製品として位置付けているか②AHA/BHA/PHA/酵素/スクラブ等の角質除去成分が主機能として配合されているか③商品説明で「角質除去」「ターンオーバー促進」「角質ケア」を主要な訴求としているか、の優先順位で判断すること。毛穴洗浄・角栓除去・クレンジング・洗顔・保湿・鎮静が主目的の製品は、多少角質除去作用があってもピーリングに分類せず絶対禁止。
- ブースター（role=booster）→ 導入美容液/導入化粧水/導入エッセンス/導入オイル(乾燥肌向け)/発酵系導入アイテム/保湿導入アイテムのみ。メーカーが導入用途として位置付けている製品に限り、通常の美容液・化粧水をブースターとして転用しない。
