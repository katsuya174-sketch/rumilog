...
以下の楽天市場の商品タイトルから、「ブランド名と製品名だけ」を抽出してください。
...
▼ 削除するもの（一切残さない）
  ...
  - 価格・割引・クーポン（500円OFF、クーポンで●円、税込●円、●%引き など）
...
weekly_care例（use_daysは種別・濃度で異なる）:
  パック→["日"]
  ピーリング（酵素系・低刺激）→[]（毎日可）または["火","木","土","日"]
  ピーリング（低濃度AHA/BHA）→["火","土"]または["水","土"]
  ピーリング（中〜高濃度AHA/BHA・物理スクラブ）→["土"]
...
■ 低濃度AHA（グリコール酸・乳酸 ＜5%）＋保湿成分豊富
  → 週2〜4回。敏感肌は週2〜3回。
  → use_days=["火","木","土"]または["水","土"]等

■ 低濃度BHA（サリチル酸 ＜2%）＋保湿成分あり
  → 週2〜3回。
  → use_days=["火","土"]または["水","土"]等

■ 中濃度AHA（5〜15%）／高濃度BHA
  → 週1〜2回。
  → use_days=["土"]または["火","土"]等

■ 物理スクラブ（スクラブ粒子・摩擦タイプ）
  → 週1〜2回（過剰摩擦でバリア損傷リスク）。
  → use_days=["土"]または["水","土"]等
...
レチノールが毎日可・ピーリングが毎日可、それぞれ独立に判断してよい。

【ルール②】ピーリングとnight刺激成分を「両方毎日」にすることは禁止
ピーリング（weekly_care）とnight刺激成分（レチノール・レチナール・AHA・BHA等）が
どちらもuse_days=[]（毎日）になる場合、必ずどちらか一方に具体的な曜日を設定すること。
- ピーリングが酵素系（毎日可）＋レチノールが毎日の場合
  → レチノールを["月","火","水","木","金","日"]、ピーリングを["土"]のように片方を特定曜日に
  → またはピーリングを["火","木","土"]等の特定曜日にして、レチノールは毎日のままにする
- 判断基準: より刺激の強い方（通常はレチノール・AHA/BHA）に曜日制限を設けるか、
  ピーリングに具体的な曜日を設定してレチノールを毎日のままにする（どちらでも可）

【ルール③】特定曜日同士が重複する場合は別の曜日へ移動
nightステップのuse_days=["月","水","金"]でレチノール使用 → ピーリングは["土"]か["日"]に設定
同一夜に刺激成分（ピーリング＋レチノール、ピーリング＋AHA/BHA等）を重ねることは肌ダメージのリスクがあるため厳禁
