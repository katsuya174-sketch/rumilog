...
- ピーリングが酵素系（毎日可）＋レチノールが毎日の場合
  → レチノールのuse_daysから一日を除いた設定にし、その日をピーリング専用日にする（例: レチノール["月","火","水","木","金","日"] + ピーリング["木"]）
  → またはピーリングを["火","木"]等の特定曜日にして、レチノールは毎日のままにする（推奨）
- 判断基準: より刺激の強い方（通常はレチノール・AHA/BHA）に曜日制限を設けるか、
  ピーリングに具体的な曜日を設定してレチノールを毎日のままにする（どちらでも可）

【ルール③】特定曜日同士が重複する場合は別の曜日へ移動
nightステップのuse_days=["月","水","金"]でレチノール使用 → ピーリングはそれらと重ならない任意の日（例: "火"や"木"や"日"等）に設定
